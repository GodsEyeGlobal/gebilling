...
class Extension extends Page implements HasActions, HasTable
{
    ...
    {
        try {
            $this->allExtensions = Cache::remember('gebilling_marketplace_extensions', now()->addHours(6), function () {
                $response = Http::timeout(15)
                    ->withUserAgent('GEBilling/' . config('app.version') . ' (https://godseyeglobal.com)')
                    ->get('https://gebapi.godseyeglobal.com/extensions', ['limit' => 999]);
                if (!$response->successful()) {
                    logger()->error('GEBilling Marketplace API request failed', ['status' => $response->status(), 'body' => $response->body()]);
            ...
            if (is_null($this->allExtensions)) {
                $this->error = 'The GEBilling Marketplace is currently unavailable. Please try again later.';
            }
        } catch (ConnectionException $e) {
            $this->error = 'Failed to connect to the GEBilling Marketplace. Please check your server\'s internet connection.';
            logger()->error('GEBilling Marketplace API connection failed: ' . $e->getMessage());
        }
    }

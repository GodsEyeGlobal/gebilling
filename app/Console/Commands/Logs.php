...
class Logs extends Command
{
    ...
    public function handle()
    {
        ...
        if (!$this->confirm('Do you want to upload the error log (including environment variables) to GEBilling Support?', true)) {
        ...
        // Add gebilling version and php version as first lines to $lastErrorMessage
        $gebillingVersion = config('app.version');
        $phpVersion = phpversion();
        $lastErrorMessage = "GEBilling Version: $gebillingVersion\nPHP Version: $phpVersion\nURL: " . url('/') . "\n\n$lastErrorMessage";

        // Post the error message to GEBilling Support
        // nc log.gebilling.org 99
        $this->info('Found error, uploading to GEBilling Support...');
        // Create a socket connection
        $socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
        socket_connect($socket, 'log.gebilling.com', 99);
    ...
    {
        // [2025-07-03 11:06:06] local.ERROR: syntax error, unexpected token "if" {"exception":"[object] (ParseError(code: 0): syntax error, unexpected token \"if\" at C:\\Users\\corwi\\Projects\\GEBilling\\GEBilling\\app\\Listeners\\InvoiceItemCreatedListener.php:18)
